...
//+---------------------------------------------------------------------+
//|Скрипт выставляет отложенные ордера на пробой максимума/минимума бара|  
//+---------------------------------------------------------------------+
...
int start()
  ...
  int tb,ts;
         ...
         if(buy==true)
            {
             SetOrder(Symbol(),OP_BUYSTOP,lot,high+spread,high+spread-StopLoss*fPointX(),0,MagicNumber,""+comment+"",0);  
            }
         else
            {
             SetOrder(Symbol(),OP_SELLSTOP,lot,low-spread,low-spread+StopLoss*fPointX(),0,MagicNumber,""+comment+"",0);
            }
         return(0);
   return(0);
  }
