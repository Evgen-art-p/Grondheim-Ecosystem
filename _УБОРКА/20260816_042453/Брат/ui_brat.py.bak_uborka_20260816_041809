...
import os
import asyncio
import base64  # PATCH_BRAT_VIZUAL_V1: кодируем картинку в data URL для мультимодального вызова
...
# PATCH_PROPISKA_BRAT — Брат связывает жителя и локацию (Брат СВЯЗЫВАЕТ, не
...
# PATCH_NAZNACHIT_ROL — Брат надевает РОЛЬ жителю (тип + маска «работа»).
...
/* PATCH_GOROD_BIRZHA_STYLE — как кнопка РЫНОК в Бирже, высота +1/3 */
...
# PATCH_BRAT_VIZUAL_V1: расширения-картинки — та же граница, что у
# Академии/Архива (KARTINKA_EXT), не заводим четвёртое название набора.
BRAT_KARTINKA_EXT = {".png", ".jpg", ".jpeg", ".webp", ".gif"}
_BRAT_KARTINKA_MIME = {".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg",
                       ".webp": "image/webp", ".gif": "image/gif"}


async def _analiz_kartinki_brata(fp: Path, model: str) -> str:
    """Визуальный просев -- та же оптика (зерно/пластик), что у
    текстового sift_file, только смотрит, а не читает. НЕ редактирует
    файл -- чистый анализ."""
    ...
    mime = _BRAT_KARTINKA_MIME.get(fp.suffix.lower(), "image/png")
    url = f"data:{mime};base64,{base64.b64encode(data).decode('ascii')}"
    soul = build_brat_soul()
    vopros = (
        f"Шеф принёс изображение на просев: {fp.name}\n\n"
        f"Пропусти через свою призму, ту же, что для текста. Напиши просев-отчёт:\n"
        f"1. ЗЕРНО — что здесь живое, настоящее, держит смысл\n"
        f"2. ПЛАСТИК — что красиво выглядит, но пусто\n"
        f"3. СВЯЗИ — как это перекликается с Грондхеймом\n"
        f"4. ВЫВОД — одна строка: что делать с этим дальше\n"
        f"Говори коротко и точно. Не выдумывай того, чего не видно на снимке."
    )
    messages = [
...
async def sift_file(fp: Path, model: str) -> str:
    ...
    PATCH_BRAT_VIZUAL_V1: изображение — визуальным разбором, текст — как раньше."""
    if fp.suffix.lower() in BRAT_KARTINKA_EXT:
        return await _analiz_kartinki_brata(fp, model)
    soul = build_brat_soul()
    content = _read(fp, limit=50000)
    prompt = (
...
def page_brat():
    ...
    async def do_propiska():
        ...
        if not zhiteli:
            ui.notify("Жителей ещё нет — роди их в Странице Жизни", color="warning")
            return
        # PATCH_PROPISKA_ZHILYE — прописываем только в жилое, не в публичные места
    ...
    with ui.element("div").classes("app-container"):
        ...
        with ui.element("div").classes("area-header"):
            with ui.element("div").classes("glass squad-deck").style(
                ...
                with ui.element("div").style(
                    ...
                    ui.button("ГРОНДХЕЙМ",
                              on_click=lambda: ui.navigate.to("/grondheim", new_tab=True)
                    ...
                    ui.button("Страница Жизни",
                              on_click=lambda: ui.navigate.to("/registry", new_tab=True)  # PATCH_ROZH_BTN
                              ).props("flat no-caps").classes("birzha-header-btn")
                ui.element("div").style("flex:1")
                with ui.element("div").style(
        ...
        with ui.element("div").classes("area-stage"):
            with ui.element("div").classes("glass stage-monitor"):
                # ── ТУЛБАР: одни врата — ГОРОД (визуальная карта) ──
                ...
                with ui.element("div").classes("stage-toolbar").style(
                    ...
                    with ui.element("div").style(
                        ...
                        ui.button("Работа",
                                  on_click=lambda: ui.navigate.to("/rabota", new_tab=True)
                                  ).props("flat").classes("brat-gate")
