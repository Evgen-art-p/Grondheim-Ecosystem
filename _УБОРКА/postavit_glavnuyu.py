...
<style>
  ...
  /* NiceGUI кладёт поверх страницы свой слой и перехватывает мышь —
     плашка видна, а дотянуться нельзя. Слой пустой (мы на нём ничего
     не рисуем), поэтому просто отключаем ему события. */
  #app, .nicegui-content, .q-page-container, .q-page {{
      pointer-events:none !important; background:transparent !important; }}
  #ostrov-vual {{ position:fixed; inset:0; z-index:1;
...
(function () {
  ...
  const v = document.getElementById('ostrov-vual');
  if (!p) return;
  const KL = 'ostrov_plashka';

  // NiceGUI монтирует своё дерево ПОСЛЕ нас и накрывает страницу.
  // Переносим вуаль и плашку в самый конец тела — тогда они сверху,
  // и мышь до них доходит.
  const naverh = () => {
    try {
      if (v) document.body.appendChild(v);
      document.body.appendChild(p);
    } catch (e) {}
  };
  naverh();
  setTimeout(naverh, 300);
  setTimeout(naverh, 1200);
