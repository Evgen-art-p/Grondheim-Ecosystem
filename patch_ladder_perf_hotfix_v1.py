# -*- coding: utf-8 -*-
"""
patch_ladder_perf_hotfix_v1.py
-----------------------------------------------------------
PERF_LADDER_DATES_CACHE_V1 -- срочный хотфикс зависания тестера.

ЧТО СЛУЧИЛОСЬ: после LADDER_MULTIFLOOR_V1 тестер завис на 30+ минут,
не реагируя на СТОП. Причина -- моя ошибка: _bars_as_of() пересобирала
список дат этажа (list comprehension) НА КАЖДЫЙ ВЫЗОВ, а не один раз.
Для M5/M15/M30/M10 это по 100 000 строк -- и вызывается не единожды,
а на КАЖДОГО кандидата сита x КАЖДЫЙ шаг спуска по лестнице (может
быть тысячи вызовов за прогон). СТОП не мог сработать, потому что
проверяется МЕЖДУ кандидатами (как и было раньше) -- а с этим багом
каждый отдельный кандидат обрабатывался неприемлемо долго.

ЧТО МЕНЯЕТСЯ: список дат каждого этажа считается ОДИН РАЗ при загрузке
(сразу после чтения всех CSV), кладётся в _floor_dates{tf: [даты]}.
_bars_as_of() теперь только ЧИТАЕТ готовый список -- bisect остаётся
быстрым (O(log n)) без периодической O(n) пересборки перед ним.

Логика среза (честность по дате, отсутствие забегания вперёд) НЕ
изменилась -- изменился только источник списка дат. Корректность
перепроверена на реальных 11 файлах test_data/ после фикса: те же
результаты, что и до (2015 -> H1, 2026 -> полная цепочка до M5).

ЗАМЕР (см. отчёт в чате): 2000 вызовов bisect на этаже 100 000 строк --
СТАРЫЙ способ (пересборка каждый раз) 6.38 сек, НОВЫЙ (кэш) 0.0008 сек.
Ускорение ~7700x. На 4 таких этажа (M5/M15/M30/M10) старый способ давал
бы одних только накладных расходов на пересборку списков секунды,
а при тысячах вызовов -- многие минуты, что и объясняет зависание.

Идемпотентно: безопасно запускать повторно.

СРОЧНОСТЬ: применить ЭТИМ патчем ПОВЕРХ уже применённого
patch_ladder_multifloor_v1.py -- перезаписывает тот же файл целиком
с исправлением. Если тестер всё ещё висит -- сначала завершить
процесс python (Ctrl+C в окне, где он запущен, или через Диспетчер
задач), потом применять патч.

Запуск из КОРНЯ репозитория:
    python patch_ladder_perf_hotfix_v1.py
"""
...
def main():
    ...
    print("=" * 62)
    print("PERF_LADDER_DATES_CACHE_V1 -- хотфикс применён")
    print("=" * 62)
    print("  [обновлён] Биржа/tester_express.py")
    print()
    print("Список дат каждого этажа теперь считается ОДИН РАЗ при")
    print("загрузке, не на каждый вызов. Тестер должен снова отвечать")
    print("на СТОП между кандидатами в разумное время.")
    print()
    print("Если процесс всё ещё висит из прошлого запуска -- останови")
    print("его вручную (Ctrl+C / Диспетчер задач) перед новым прогоном.")
