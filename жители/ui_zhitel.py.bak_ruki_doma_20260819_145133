...
import sys
import asyncio
import base64  # PATCH_ZHITEL_VIZUAL_V1: кодируем картинку в data URL для мультимодального вызова
from pathlib import Path
from datetime import datetime

from nicegui import ui, events  # ZHITEL_CHTENIE_V1: events для handle_upload
...
# PATCH_ZHITEL_VIZUAL_V1: та же граница расширений, что у Академии/Архива
KARTINKA_EXT = {".png", ".jpg", ".jpeg", ".webp", ".gif"}
_KARTINKA_MIME = {".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg",
                  ".webp": "image/webp", ".gif": "image/gif"}


def _prompt_chtenia_kartinka(imya_fajla: str, linza: str,
                             lok_imya: str, professia: str = "") -> str:
    """PATCH_ZHITEL_VIZUAL_V1: та же линза МЕСТО × РОЛЬ, что у
    _prompt_chtenia, но для изображения — вопрос без CONTENT (картинка
    идёт отдельным куском мультимодального сообщения)."""
    if linza == "работа":
        ...
        zadacha = (f"Посмотри на это изображение {_rol}: вытащи структурную "
                   "пользу — что видно, какие детали, схемы, приёмы пополнят "
                   "твой профессиональный арсенал.")
    else:
        gde = ("Ты сейчас дома, смотришь для себя, в своём ритме."
               if not lok_imya
               else f"Ты сейчас на «{lok_imya}», но роли тебе тут не дано — "
                    "ты здесь гость и смотришь для себя, в своём ритме.")
        zadacha = ("Посмотри на это изображение как человек для себя: что "
                   "видно, что тронуло, какие мысли и смыслы отозвались.")
    return (
        f"{gde}\nТебе принесли изображение: {imya_fajla}\n\n"
        f"{zadacha}\n"
        f"Напиши концентрат увиденного от первого лица: 5–12 плотных строк — "
        f"суть плюс твой личный отклик. Не выдумывай того, чего не видно. "
...
def _prompt_chtenia(imya_fajla: str, tekst: str, linza: str,
                    lok_imya: str, professia: str = "") -> str:
...
def page_zhitel(zid: str = ""):
    ...
    state = {"chat": [], "model": DEFAULT_MODEL}
    ...
    async def do_chtenie():
        ...
        try:
            ...
            return
        state["waiting"] = True
        linza, lok_imya, professia = _linza_chtenia(dom)  # ZHITEL_CHTENIE_MASKA_V1
        _kto = (f" — как {professia} на «{lok_imya}»" if linza == "работа"
        ...
        for fp in fajly:
            # PATCH_ZHITEL_VIZUAL_V1: изображение — визуальный разбор той
            # же линзой; текст — как раньше, через _prochitat_fail.
            if fp.suffix.lower() in KARTINKA_EXT:
                try:
                    _img_data = fp.read_bytes()
                except Exception as _e:
                    ui.notify(f"⚠ {fp.name}: не прочитать файл — {_e}", color="negative")
                    continue
        ...
        try:
            ...
            pass
        # PATCH_ZHITEL_FIX_WAITING_V1: эти три строки случайно потерялись
        # при вставке do_prosev() более ранним патчем -- без них
        # state["waiting"] оставался True навсегда после "Прочитать",
        # и send()/do_prosev() молча переставали отвечать.
        state["waiting"] = False
        update_files()
        update_chat()
        ...
        with ui.element("div").classes("area-left"):
            with ui.element("div").classes("left-col"):
                with ui.element("div").classes("glass").style("flex:1; overflow:hidden;"):
                    ...
                    ui.upload(on_upload=handle_upload, multiple=True, auto_upload=True).props("flat color=amber").style("margin:8px;")  # ZHITEL_CHTENIE_V1
