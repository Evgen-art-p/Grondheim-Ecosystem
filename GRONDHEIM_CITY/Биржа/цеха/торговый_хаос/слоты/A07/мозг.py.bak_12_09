...
# ─────────────────────────────────────────────────────────────
# ЖИВОЙ ПРОГОН ТРЕЙДЕРА (A07) — второй ТРЕЙДЕР Совета Биржи
# AVAN_ENGINE_V1 · перенесён на слотовое шасси (тот же приём, что трейдер)
#
...
STATS_PATH   = STATE_DIR / "stats_A07.json"
DIARY_PATH   = STATE_DIR / "diary_A07.jsonl"
...
def _append_diary(signal: dict, diary_entry: dict, market: dict, table: dict):
    ...
    event = _podpisat({
        ...
        "что":       signal.get("avan_action"),
        "почему":    signal.get("avan_reason"),
        "рычаг":     bool(signal),
        "ключ":      _klyuch_svoy(),
...
def run_avan(symbol: str = "XAUUSD", timeframe: str = "H4",
    ...
    # импорт падал всегда, soul='' , торговал 'трейдер-вообще'.
    ...
    user_msg = (
        ...
        "стоп, какой lot — твоя рука, не рельса. Не работаешь — отдай "
        "WAIT рукой. Никто не подложит тебе готовую цену и не скажет, как "
        "поступить.\n\n"
...
def _signal_ot_ruki(bar_time=None, slova: dict = None) -> dict:
    ...
    v = dict((t.get(_TABLO_KEY_AVAN) or {}))
    ...
    for pole in _PRIKAZ_POLYA:
        if v.get(pole) is not None:
            out["avan_" + pole] = v[pole]
    print(f"[РЕШЕНИЕ] с руки: {out.get('avan_action')} "
          f"{out.get('avan_direction') or ''}".rstrip())
    return out
