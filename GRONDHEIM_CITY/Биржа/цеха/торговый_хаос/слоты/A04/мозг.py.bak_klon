...
def chat_with_hans(question: str, last_run: Optional[dict] = None,
    ...
    try:
        ...
        soul = format_soul_for_agent("A04_GANS", dept="trading")
        if soul:
            system = prompt + "\n\n=== ТВОЁ СОСТОЯНИЕ (душа) ===\n" + soul + "\n\n" + work_ctx
    except Exception:
        pass
    ...
    try:
        return chat(system=system, user=question, history=history,
                    agent_id="A04_GANS", slot_id="trading")
...
def run_hans(symbol: str = "XAUUSD", timeframe: str = "H4",
    ...
    try:
        ...
        print(f"[HANS] ⚠️  Душа не загрузилась ({e}) — работаю без неё")
    ...
    try:
        ...
        if says_valid and fact_valid:
            sync_to_dna("A04_GANS", "good_work", intensity=0.5, dept="trading")
        elif says_valid and not fact_valid:
            # объявил добычу там, где факт движка её не видит — соврал
            sync_to_dna("A04_GANS", "bad_work", intensity=0.5, dept="trading")
        # says_valid=False — честный ноль, петлю не трогаем
    except Exception as e:
        print(f"[HANS] ⚠️  sync_to_dna не сработал ({e})")
