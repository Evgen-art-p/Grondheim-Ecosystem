# -*- coding: utf-8 -*-
"""
dve_gorki_matryoshka.py — линии расхождения по экстремумам, на всех размерах.

Слово Шефа 23.09: «линии должны тянуться по экстремумам, на цене и AO:
самый экстремум AO и следующий меньше, и смотрим». И по его скрину
USDCNH: после отката пятая волна несёт свой дивер внутри.

Что делает:
  1. Биржа/sverka_divera.py — функция dve_gorki заменяется целиком.
     Одна пара: самый экстремум AO; откат цены делит ход надвое; самый
     глубокий AO после отката; цена — самый низ до и после отката.
     Матрёшка: вторая точка пары становится первой точкой следующей,
     меньшей, — и так до края. В лог — весь ход и край.
  2. Биржа/grafik.py — учебные линии: толстая — самая большая пара
     (весь ход), тонкая — последняя справа (край). Одна пара — одна
     линия.

Заменяет и старую версию, и dve_gorki_ekstremumy.py, если он уже стоит.
...
сперва проверяет всё, потом пишет. Копии `.bak_matryoshka`.
...
METKA_SV = "DVE_GORKI_MATRYOSHKA_V1"
...
def main():
    ...
    for put, novyy, staryy, crlf in ((p_sv, n_sv, t_sv, crlf_sv),
        ...
        try:
            ...
            return
        print(f"✓ {put}\n  копия до правки: {bak}")
    print("  Линии по экстремумам на всех размерах: толстая — весь ход, "
          "тонкая — край.")
