...
# ════════════════════════════════════════════════════════════
def uslovie_otkat_do_urovnya(bars: list[dict], i: int, indeks_tochki: int,
                              storona: str, uroven: float) -> dict | None:
    """
    ЧЕРНОВИК, не канон — сам критерий отката (простое касание уровня,
    или глубина по Фибо, или что-то ещё) это решение Шефа, не моё.
    Здесь — самый грубый вариант: касание уровня Брута для честной
    первой проверки механизма, не для боевого применения.

    storona "up"   → импульс вверх, откат = движение цены ВНИЗ до уровня.
    storona "down" → откат = движение цены ВВЕРХ до уровня.
    """
    bar = bars[i]
    if storona == "up" and bar["low"] <= uroven:
        return {"otkat_price": bar["low"], "otkat_date": bar["date"]}
    if storona == "down" and bar["high"] >= uroven:
        return {"otkat_price": bar["high"], "otkat_date": bar["date"]}
    return None
