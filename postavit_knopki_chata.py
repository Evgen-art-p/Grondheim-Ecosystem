...
═══ ЗАОДНО ЧИНИТСЯ СТАРЫЙ ДОЛГ ═══

`nositel.zapisat_vyvod` и `zapisat_vyvod_pare` пишут рыночный вывод
голосом `otkuda="рынок"`. А рабочими контекстами считаются «работа» и
«факт» — «рынка» среди них нет. То есть **собственные выводы трейдера
после взглядов рабочим поиском не находятся**. Долг был записан ещё
04.09 как «контекст не проставляется»; вот он, в двух строках.
Меняем на «работа» в обоих местах.
...
`Биржа/nositel.py`:
  3. Голос рыночного вывода «рынок» → «работа» (два места).
...
def _pravki_nositel(text: str):
    n = text.count(GOLOS_OLD)
    if n != 2:
        return None, f"голос вывода найден {n} раз (ожидалось 2)"
    return text.replace(GOLOS_OLD, GOLOS_NEW), None


def main() -> None:
    root = Path(__file__).resolve().parent
    zadachi = [(root / UI, _pravki_ui), (root / NOSITEL, _pravki_nositel)]
